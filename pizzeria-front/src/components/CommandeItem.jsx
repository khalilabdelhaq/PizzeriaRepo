...
export default class CommandeItem extends Component {
  ...
  detailCommande(){
    ...
    render() {
      ...
      return (
        <tr key={this.props.id}><td>{nomClient}</td><td>{prenomClient}</td><td>{adresse}</td><td>{tel}</td><td>{typePizza}</td><td>{taillePizza}</td><td>{saucePizza}</td><td>{quantite}</td><td><button className="previous" onClick={this.livrerCommande}>Livrer</button><button className="next" onClick={this.detailCommande}>Détail</button></td></tr>
      );
    }
  }